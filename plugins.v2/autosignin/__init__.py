...
class AutoSignIn(_PluginBase):
    ...
    def __do(self, today: datetime, type_str: str, do_sites: list, event: Event = None):
        ...
        # 清理14天前的旧数据，保留近期历史供详情页展示
        old_date = today - timedelta(days=14)
        old_str = old_date.strftime('%Y-%m-%d')
        self.del_data(key=type_str + "-" + old_str)
        self.del_data(key=f"{old_date.month}月{old_date.day}日")
        ...
        if status:
            ...
            for s in status:
                ...
                if site_name:
                    site_id = sites.get(site_name)
                if site_id and site_id not in retry_sites:
                    success_sites.append(site_id)

            # 合并本次已处理站点与历史已成功站点（兼容重试场景）
            if not self._clean and today_history:
                already_sites = today_history.get("do") or []
                success_sites = list(set(success_sites + already_sites))

            logger.debug(f"本次{type_str}成功站点 {success_sites}")
